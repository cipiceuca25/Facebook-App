...
$(document).ready(function() {
	...
	/*
	 * if(images.length > 0){ setTimeout( function(){; getBadgeNotification(); },
	 * 3000); }
	 */
});
		...
		.on(
				...
				function() {
					...
					if (document.addEventListener
							&& this.scrollHeight > parseInt($(this).css(
									'max-height'))) { // Firefox only
 ...
 * console.log('hover on'); if(window.addEventListener){ //Firefox only
 * window.addEventListener("DOMMouseScroll", function(e){e.preventDefault()},
 * true); window.addEventListener("mousewheel", function(e){e.preventDefault()},
 * true); }
 * 
 * }); $(document).on('mouseleave', '.comments.popup_scoll', function (){
 * console.log('hover off'); if(window.addEventListener){ //Firefox only
 * window.addEventListener("DOMMouseScroll", function(e){console.log('trying to
 * move');return true}, true); window.addEventListener("mousewheel",
 * function(e){console.log('trying to move');return true}, true); }
 * 
 * });
 */
// ===========================================================================================
...
function MouseWheelHandler() {
	...
	FB.Canvas.getPageInfo(function(info) {
		...
		if (info.scrollTop > 500 + x) {
			...
			$('#info-box-container').animate({
				'top' : info.scrollTop - 500
			}, 150, function() {
			});
	...
	$
			.ajax({
				...
				beforeSend : function() {
					$('.profile-content')
							.html(
									"<div class='rvrow'><div class=' rvgrid-11 '><div class=' box '><div style='text-align:center; padding:40px 0 40px 0'><img src='/img/ajax-loader.gif' /></div></div></div></div>");
				},
				success : function(data) {
...
function feedbackAnimation(ui, type) {
	...
	case 'like':
		$(ui)
				.before(
						'<div class = "like-animation" style="width:60px; left:-7px; z-index:100;"></div>')
		setTimeout(function() {
	...
	case 'unlike':
		$(ui)
				.before(
						'<div class = "unlike-animation" style="width:75px; left:-11px; z-index:100;"></div>')
		setTimeout(function() {
	...
	case 'follow':
		$(ui)
				.before(
						'<div class = "follow-animation" style=" left:-20px; width:86px; z-index:100;"></div>')
		setTimeout(function() {
	...
	case 'unfollow':
		$(ui)
				.before(
						'<div class = "unfollow-animation" style="left:-38px; width:102px;z-index:100;"></div>')
		setTimeout(function() {
...
function userProfile(user, load) {
	...
	$
			.ajax({
				type : "GET",
				url : serverUrl + '/app/app/userprofile/' + fanpageId
						+ '/?target=' + user,
				dataType : "html",
				cache : false,
				async : true,
...
function getBadgesList(ui, type) {

	$
	.ajax({
		type : "GET",
		url : serverUrl + '/app/app/badgeslist/' + fanpageId
				+ '/?type=' + type,
		dataType : "html",
		cache : false,
		async : true,
...
function getUpcomingBadges(ui, limit) {
	...
	.ajax({
		type : "GET",
		url : serverUrl + '/app/app/upcomingbadges/' + fanpageId
				+ '/?limit=' + limit + '&notifier=false',
		dataType : "html",
		cache : false,
		async : true,
		beforeSend : function() {
			...
			if (ui == '#profile_upcoming_badges') {
				$(ui)
						.html(
								"<div style='text-align:center;margin-top:-1px; padding: 0 25px 0 ' ><img src='/img/ajax-loader.gif' /></div>");
			} else {
				$(ui)
						.html(
								"<div style='text-align:center;margin-top:-1px; padding: 0 25px 0 ' ><img src='/img/ajax-loader-white.gif' /></div>");
	...
	$
			.ajax({
				...
				url : serverUrl + '/app/app/fancrankfeedcomment/' + fanpageId
						+ '?post_id=' + post_id + '&post_type=' + type +

						'&total=' + total + '&latest=' + latest + '&filter='
						+ filter + '&popup=' + popup,
				dataType : "html",
				cache : false,
				async : true,
				beforeSend : function() {
					...
					$(ui)
							.html(
									"<div class='comments' style='text-align:center;'><li class='comment-container'><img src='/img/ajax-loader.gif' /></li></div>");
				},
				success : function(data) {
					$(ui).html(data);
					changeTime(ui + ' .time');
...
function popup_post(post_id, load) {
	...
	$
			.ajax({
				type : "GET",
				url : serverUrl + '/app/app/popuppost/' + fanpageId
						+ '?post_id=' + post_id,
				dataType : "html",
				cache : false,
				async : true,
	...
	$
			.ajax({
				...
				beforeSend : function() {
					$('#news-feed')
							.html(
									"<div style='text-align:center; padding:10px 0 40px 0'><img src='/img/ajax-loader.gif' /></div>");
				},
				success : function(data) {
	...
	$
			.ajax({
				...
				beforeSend : function() {
					$('#toppost')
							.html(
									"<div style='text-align:center; padding:40px 0 40px 0'><img src='/img/ajax-loader.gif' /></div>");
				},
				success : function(data) {
...
function like(post_id, post_type, target_id, target_name) {
	...
	if (post_type.indexOf('comment') != -1) {
		mes = $('.comment-container.' + post_id + ' .user .message').text()
				.substring(0, 99);
	} else {
		mes = $('.post-container.' + post_id + ' .post .message').text()
				.substring(0, 99);
	}

	$.ajax({
		type : "GET",
		url : serverUrl + '/app/user/' + userId + '/likes/?post_id=' + post_id
				+ '&fanpage_id=' + fanpageId + '&post_type=' + post_type
				+ '&target_id=' + target_id + '&target_name=' + target_name
				+ '&access_token=' + userAccessToken + '&mes=' + mes,
		dataType : "json",
		cache : false,
		async : true,
		success : function(data) {
			...
			// addActivities('like-' + post_type, post_id, target_id,
			// target_name, mes);
			...
			if (!isNaN(data.fan_point)) {
				...
				$('.my_fan_point_tooltip').attr('data-original-title',
						'You have ' + data.fan_point + ' points');
			}
			...
			$('.like_control_' + post_id).attr('data-original-title',
					'You like this');

			temp = $('.social.like.' + post_id + ' a').attr(
					'data-original-title');

			if (num == 1) {
				$('.social.like.' + post_id + ' a').attr('data-original-title',
						target_name + ' likes this');
			} else {
				$('.social.like.' + post_id + ' a').attr('data-original-title',
						target_name + ', ' + temp);
			}
...
function unlike(post_id, post_type, target_id, target_name) {
	...
	$.ajax({
		...
		url : serverUrl + '/app/user/' + userId + '/unlike/?post_id=' + post_id
				+ '&fanpage_id=' + fanpageId + '&post_type=' + post_type
				+ '&target_id=' + target_id + '&target_name=' + target_name
				+ '&mes=' + mes + '&access_token=' + userAccessToken,
		...
		success : function(data) {
			...
			// addActivities('unlike-' + post_type, post_id,target_id,
			// target_name, mes);
			...
			$('.like_control_' + post_id).attr(
					'onclick',
					"like('" + post_id + "','" + post_type + "','" + target_id
							+ "','" + target_name + "')");
...
function getFancrankfeed(view) {
	...
	// alert(serverUrl + '/app/app/fancrankfeed/' + fanpageId + '?viewAs='+ view
	// + '&limit=' + feedLimit);
	// alert(setFeed);
	// alert(feedLimit);
	// if (view != 'myfeed'){
	...
	$
			.ajax({
				type : "GET",
				url : serverUrl + '/app/app/fancrankfeed/' + fanpageId
						+ '?viewAs=' + view + '&until=' + last,
				dataType : "html",
				cache : false,
				async : true,
				beforeSend : function() {
					...
					if ((setFeed != view)) {
						$('#fancrankfeed')
								.html(
										"<div id='loader' style='text-align:center; padding:10px 0 40px 0'><img src='/img/ajax-loader.gif' style='margin-top:20px'/></div>");
					} else {
						$('#more_post').remove();
						$('#fancrankfeed')
								.append(
...
function getTopFan() {
	...
	$
			.ajax({
				type : "GET",
				url : serverUrl + '/app/app/topfan/' + fanpageId
						+ '?facebook_user_id=' + userId,
				dataType : "html",
				cache : false,
				async : true,
	...
	$
			.ajax({
				...
				beforeSend : function() {
					$(ui)
							.append(
									"<div class='removal' style='text-align:center; padding:40px 0 40px 0'><img src='/img/ajax-loader.gif' /></div>");
				},
				success : function(data) {
					$(ui + ' .removal').remove();
					$(ui).append(data);
			...
			.ajax({
				...
				success : function(data) {
					...
					if (data.length < 1) {
						...
						getBadgesList('#display_badges', 'chosen') ;
						getBadgesList('#profile_earned_badges', 'earned') ;
						getBadgesList('#profile_redeem_badges', 'redeemed') ;
					}
				},
				error : function(xhr, errorMessage, thrownErro) {
					console.log(xhr.statusText, errorMessage);
...
function getRedeem() {
	$
			.ajax({
				type : "GET",
				url : serverUrl + '/app/app/redeem/' + fanpageId
						+ '?facebook_user_id=' + userId,
				dataType : "html",
				cache : false,
				async : true,
	...
	$
			.ajax({
				...
				beforeSend : function() {
					$('.notifier')
							.html(
									"<div class='arrow' style='left:140px'></div><div class='rvgrid-7' style='text-align:center;'><div class='box'><img src='/img/ajax-loader-white.gif' /></div></div>");
				},
				success : function(data) {
					$('.notifier').html(data);
	...
	$
			.ajax({
				...
				beforeSend : function() {
					...
					$('.notifier')
							.html(
									"<div class='arrow' style='left:190px'></div><div class='rvgrid-7' style= text-align:center;'><div class='box'><img src='/img/ajax-loader-white.gif' /></div></div>");
				},
				success : function(data) {
					$('.notifier').html(data);
...
function getRelation(target, ui) {
	$.ajax({
		type : "GET",
		url : serverUrl + '/app/user/' + userId + '/relation/?target_id='
				+ target + '&fanpage_id=' + fanpageId,
		dataType : "html",
		cache : false,
		success : function(data) {
			data = $.trim(data);
			...
			success : function(data) {
				...
				$('.' + ui).attr(
						...
						"unfollow('" + target + "','" + name + "','" + ui
								+ "')");
				$('.' + ui).attr('data-original-title',
						'Click to Unfollow this User');
				// $('.'+ui).html('<span class="badge
				// badge-'+relation+'">'+relation+'</span>');
...
function unfollow(target, name) {
	...
	$.ajax({
		...
		url : serverUrl + '/app/user/' + userId + '/unfollow/?subscribe_to='
				+ target + '&target_name=' + name + '&fanpage_id=' + fanpageId
				+ '&subscribe_ref_id=1',
		dataType : "html",
		cache : false,
		success : function(data) {
			...
			$('.' + ui)
					.attr('data-original-title', 'Click to Follow this User');
 ...
 * $.ajax({ type : "GET", url : serverUrl + '/app/user/' + userId +
 * '/addactivity/?owner_name=' + userName + '&activity_type=' + act_type +
 * '&event=' + event + '&fanpage_id=' + fanpageId + '&target_id=' + target_id +
 * '&target_name=' + target_name + '&message=' + message, dataType : "html",
 * cache : false, async: true, success : function(data) { console.log('activity
 * recorded'); //alert('adding act'); }, error : function(xhr, errorMessage,
 * thrownErro) { console.log(xhr.statusText, errorMessage); console.log('issue
 * with activity save'); } });
 *  }
 */
...
function post(button) {
	...
	if (mes == '' || mes == null) {
		...
		$
				.ajax({
					...
					url : serverUrl + '/app/user/' + userId
							+ '/post/?fanpage_id=' + fanpageId
							+ '&access_token=' + userAccessToken
							+ '&fanpage_name=' + fanpageName + '&message='
							+ mes,
					dataType : "html",
					cache : false,
					async : true,
		...
		// addActivities('post-status', response.id, fanpageId,
		// fanpage_name,mes.substring(0,99) );
		$('#post_box').val('');
		getFancrankfeed('post');
	}
}

function commentSubmit(button, post_id, post_type, post_owner_id,
		post_owner_name, isLatestAdminPost) {
	...
	if (mes == '' || mes == null) {
		...
		$
				.ajax({
					type : "GET",
					url : serverUrl + '/app/user/' + userId
							+ '/comment/?post_id=' + post_id + '&post_type='
							+ post_type + '&target_id=' + post_owner_id
							+ '&target_name=' + post_owner_name
							+ '&fanpage_id=' + fanpageId + '&fanpage_name='
							+ fanpageName + '&message=' + mes
							+ '&access_token=' + userAccessToken,
					dataType : "html",
					cache : false,
					async : true,
					...
					success : function(data) {
						// addActivities('comment-' + post_type, post_id,
						// post_owner_id, post_owner_name, mes.substring(0,99));
						$('#comment_box_' + post_id).val('');

						post_comment_count = parseInt($('#post_' + post_id)
								.attr('data-comment-count')) + 1;
						//console.log(post_comment_count);
						// alert(post_comment_count);
						$('.comment_' + post_id).attr('data-comment-count',
								post_comment_count);
						$('.comment_' + post_id).html(' ' + post_comment_count);

						if (isLatestAdminPost) {
							comment_feed2(post_id, post_type,
									post_comment_count, false);
						} else {
							comment_feed(post_id, post_type,
									post_comment_count, false);
						}
					},
					error : function(xhr, errorMessage, thrownErro) {
...
function commentSubmit2(post_id, post_type, post_owner_id, post_owner_name) {
	...
	if (mes == '' || mes == null) {
		...
		$.ajax({
			type : "GET",
			url : serverUrl + '/app/user/' + userId + '/comment/?post_id='
					+ post_id + '&post_type=' + post_type + '&target_id='
					+ post_owner_id + '&target_name=' + post_owner_name
					+ '&fanpage_id=' + fanpageId + '&fanpage_name='
					+ fanpageName + '&message=' + mes,
			dataType : "html",
			cache : false,
			...
			success : function(data) {
				// addActivities('comment-' + post_type, post_id, post_owner_id,
				// post_owner_name, mes.substring(0,99));
				$('#comment_box_popup_' + post_id).val('');

				post_comment_count = parseInt($('#post_' + post_id).attr(
...
function getLikesList(postid, limit, load) {
	...
	$
			.ajax({
				type : "GET",
				url : serverUrl + '/app/app/getlikeslist/' + fanpageId
...
function changeTime(ui) {
	$.each($(ui), function(index) {
		...
		$(this).attr('data-original-title',
				timeZoneTitle($(this).attr('data-unix-time')));
	});
}
		...
		if (daydiff == 1) {
			...
			if (hourdiff < 1) {
				...
				if (mindiff < 1) {
					...
					return secdiff
							+ ((secdiff == 1) ? ' second ago' : ' seconds ago');
				}

				return mindiff
						+ ((mindiff == 1) ? ' minute ago' : ' minutes ago');
			}
...
function editdescription() {
	...
	$('.user-description')
			.html(
					"<textarea id='user-description-box'>"
							+ $.trim(msg)
							+ "</textarea><a class='user-description-button' onclick='submit_user_description()'><span class='btn btn-mini fc-edit'>Save Description</span></a>");
	$('.user-description')
			.after(
					'<div id="charcount" style="margin-left:9px; color:#bbb">160 Characters Left</div>');
	charcheck();
...
function submit_user_description() {
	...
	if ((msg).length > 160) {
		...
		$('.user-description')
				.after(
						'<a class="user-description-button" onclick="editdescription()"><span class="btn btn-mini fc-edit-2"><img src="/img/icons/edit_icon.png" class="edit_icon" />Edit Description</span></a>');
		$('#charcount').remove();
		$.ajax({
			type : "GET",
			url : serverUrl + '/app/user/' + userId + '/saveuserdescription/'
					+ '?fanpage_id=' + fanpageId + '&message=' + $.trim(msg),
...
var tourOptions = {
	welcomeMessage : '<h3>Welcome to FanCrank</h3><p>Hi '
			+ userName
			+ ', <br/> Let\'s learn about using FanCrank. <br/> Click Start to begin</p>',
	data : [
			// -1
			{
				element : '#pageTabs',
			...
			// 7
			{
				...
				'text' : 'Latest Posts will always show the most recent post of the '
						+ fanpageName + ' has made.<br/><br/>'
			},
			// 8
			{
			...
			// 18
			{
				...
				'text' : 'Here you can choose how you want to view your posts, All, My Feed or Page Post <br/> \
					   		...
					   		Page Post is show only post made by <span style="font-weight:bold">'
						+ fanpageName + '</span \
					   		<br/><br/>'
			},
			// 19
			{
				...
				'text' : 'If you want to post something through FanCrank, you can by simply typing here and clicking Share <br/>'
						+ 'Share will only show up after you\'ve tried to type something'
						+ '<br/><br/>'
			},
			// 20
			{
...
function submit_badges_choice() {
	...
	$.ajax({
		type : "GET",
		url : serverUrl + '/app/user/' + userId + '/savechosenbadges/' + '?c1='
				+ sbadges[0] + '&c2=' + sbadges[1] + '&c3=' + sbadges[2]
				+ '&fanpage=' + fanpageId,
		dataType : "html",
		cache : false,
		async : true,
...
/*
 * function isLogin(data) { if($.trim(data) == "") { alert('hi');
 * //window.location.href = '/app/index/index/'+fanpageId; return false; }else {
 ...
 * fileUpload(form, action_url, div_id); addActivities('post-photo', userName,
 * fanpageId, target_id, target); setFeed = 0; getFancrankfeed(setFeed); }
 * 
 * function fileUpload(form, action_url, div_id) {
 *  // Create the iframe... var iframe = document.createElement("iframe");
 * iframe.setAttribute("id", "upload_iframe"); iframe.setAttribute("name",
 * "upload_iframe"); iframe.setAttribute("style", "width: 100; height: 50;
 * border: 1; display:block");
 *  // Add to document... form.parentNode.appendChild(iframe);
 * window.frames['upload_iframe'].name = "upload_iframe";
 * 
 * iframeId = document.getElementById("upload_iframe");
 *  // Add event... var eventHandler = function() {
 * 
 * if (iframeId.detachEvent) iframeId.detachEvent("onload", eventHandler); else
 * iframeId.removeEventListener("load", eventHandler, false);
 *  // Message from server... if (iframeId.contentDocument) { content =
 * iframeId.contentDocument.body.innerHTML; } else if (iframeId.contentWindow) {
 * content = iframeId.contentWindow.document.body.innerHTML; } else if
 * (iframeId.document) { content = iframeId.document.body.innerHTML; }
 * 
 * document.getElementById(div_id).innerHTML = content;
 *  // Del the iframe... setTimeout('iframeId.parentNode.removeChild(iframeId)',
 * 250); };
 * 
 * if (iframeId.addEventListener) iframeId.addEventListener("load",
 * eventHandler, true); if (iframeId.attachEvent) iframeId.attachEvent("onload",
 * eventHandler);
 *  // alert('now im gonna send the form'); action_url = action_url +
 * '&message=' + $('#post_box').val(); action_url = action_url +
 * '&access_token=' + userAccessToken; alert(action_url); // Set properties of
 * form... form.setAttribute("target", "upload_iframe");
 * form.setAttribute("action", action_url); form.setAttribute("method", "post");
 ...
 *  }
 * 
 * function colorChange(choice) { $.ajax({ type : "GET", url : serverUrl +
 * '/app/user/' + userId + '/color/?choice=' + choice + '&fanpage_id=' +
 * fanpageId, dataType : "html", cache : false, success : function(data) {
 * window.location.reload();
 * 
 * //$(ui).html(data); }, error : function(xhr, errorMessage, thrownErro) {
 * console.log(xhr.statusText, errorMessage); } });
 *  }
 * 
 * 
 * $('#achievements-tab').live('click', function() { getAwards();
 * $('#leaderboard').html(''); $('#profile').html(''); $('#news-feed').html('');
 * $('#redeem').html(''); FB.Canvas.setSize({ width : 810, height : 600 });
 * 
 * });
 * 
 * function getAwards() { $.ajax({ type : "GET", url : serverUrl +
 * '/app/app/awards/' + fanpageId + '?facebook_user_id=' + userId, dataType :
 * "html", cache : false, async : true, beforeSend: function(){
 * $('#achievements').html("<div style='text-align:center; padding:10px 0 40px
 * 0'><img src='/img/ajax-loader.gif' /></div>"); }, success : function(data) {
 * $('#achievements').html(data);
 *  }, error : function(xhr, errorMessage, thrownErro) {
 * console.log(xhr.statusText, errorMessage); console.log('error getting the
 * awards page'); } }); }
 * 
 */
...
$(document)
		.ready(
				function() {
					...
					$('#updateShippingConfirm').live(
							'click',
							function(e) {
								e.preventDefault();

								var formData = $('#update-shipping-form')
										.serialize();
								//console.log(formData + selectBadgeId);

								if (!$('#update-shipping-form').valid()) {
									$('#update-shipping-form').validate()
											.focusInvalid();
								} else {
									$.ajax({
										type : "POST",
