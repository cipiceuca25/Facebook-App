...
function saveLeaderboard() {
	...
	try {
		foreach ($fanpageList as $fanpageId) {
			...
			if(!empty($result)) {
				insertLog($fanpageId, $result, 'Top-Fan-Week', 'count');
			...
			if(!empty($result)) {
				insertLog($fanpageId, $result, 'Top-Clicker-Week', 'number_of_likes');
			}
			
			//save top talker
			$result = $rankingModel->getTopTalkerByWeek($fanpageId, 5);
			if(!empty($result)) {
				insertLog($fanpageId, $result, 'Top-Talker-Week', 'number_of_posts');
			}
			
			//save most popular
			$result = $rankingModel->getMostPopularByWeek($fanpageId, 5);
			if(!empty($result)) {
				insertLog($fanpageId, $result, 'Top-Popular-Week', 'count');
			}
			
			//save top follower
			$result = $rankingModel->getTopFollowedByWeek($fanpageId, 5);
			if(!empty($result)) {
				insertLog($fanpageId, $result, 'Top-Follow', 'count');
			}
		}
		//
	} catch (Exception $e) {
