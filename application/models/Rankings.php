...
class Model_Rankings extends Model_DbTable_Rankings
{
	...
	protected $_firstdayOfTheMonth;
	protected $_firstdayOfLastMonth;
	...
	public function __construct() {
		...
		$this->_firstdayOfTheMonth = Fancrank_Util_Date::firstdayOfTheMonth();
		$this->_firstdayOfLastMonth = Fancrank_Util_Date::firstdayOfLastMonth();
	}
	...
	public function getTopFansByLastMonth($page_id, $limit = 5) {
	...
	public function getUserTopFansRankByMonth($fanpage_id, $facebook_user_id) {
		$select = "select *
				...
				(
					...
					left join fans f
					on f.facebook_user_id = p.facebook_user_id
					where f.fanpage_id = $fanpage_id && f.facebook_user_id != f.fanpage_id && p.created_time > '$this->_firstdayOfTheMonth'
					group by f.facebook_user_id
					having f.facebook_user_id not in (Select facebook_user_id from fanpage_admins where fanpage_id = $fanpage_id)
					order by count DESC
				) as rank, (SELECT @rownum:=0) r
