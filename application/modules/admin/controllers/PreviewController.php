 ...
 * @license     
 */
class Admin_PreviewController extends Fancrank_Admin_Controller_BaseController {
	protected $_fanpage;
	...
	public function preDispatch() {
		...
		if (! empty ( $this->_fanpageId ) && ! $fanpageAdminModel->findRow ( $this->_identity->facebook_user_id, $this->_fanpageId )) {
			$this->_helper->redirector ( 'index', 'index' );
		}
		
		$fanpageModel = new Model_Fanpages ();
		
		$this->_helper->layout ()->setLayout ( 'preview_landingpage_layout' );
		$this->_fanpage = $fanpageModel->findRow ( $this->_fanpageId );
		$this->_facebook_user = $this->_identity;
		$this->_fanpageProfile = $this->_fanpage;
		$this->_accessToken = $this->_fanpage->access_token;
		$this->_userId = $this->_facebook_user->facebook_user_id;
		
		if (empty ( $this->_fanpage )) {
			exit ();
		}
		...
		$this->view->fanpage_name = $this->_fanpage->fanpage_name;
		$this->view->fanpage = $this->_fanpage;
	...
	public function landingpageAction() {
		...
		try {
			if (isset ( $cache ) && ! $cache->load ( $this->_fanpageId . '_topfan_lasttime' )) {
				$topfanLasttime = $model->getTopFansByLastMonth ( $this->_fanpageId, 5 );
				$cache->save ( $topfanLasttime, $this->_fanpageId . '_topfan_lasttime' );
			} else {
				$topfanLasttime = $cache->load ( $this->_fanpageId . '_topfan_lasttime' );
			}
			
			if (isset ( $cache ) && ! $cache->load ( $this->_fanpageId . '_topfan' )) {
				$topfan = $model->getTopFansByCurrentMonth ( $this->_fanpageId, 5 );
				$cache->save ( $topfan, $this->_fanpageId . '_topfan' );
			} else {
				$topfan = $cache->load ( $this->_fanpageId . '_topfan' );
			}
		} catch ( Exception $e ) {
			Zend_Registry::get ( 'appLogger' )->log ( $e->getMessage () . ' ' . $e->getCode (), Zend_Log::NOTICE, 'memcache info' );
			// echo $e->getMessage();
		...
		$isEnabled = $fp_settings->isProfileImageEnable ( $this->_fanpageId );
		$imageURL = $fp_settings->profileImageUrl ( $this->_fanpageId );
		$this->view->is_enable = $isEnabled;
		$this->view->image_url = $imageURL;
		$this->view->top_fans = $topfan;
		$this->view->top_fans_lasttime = $topfanLasttime;
	...
	public function topfanAction() {
		...
		/*
		 * $user = $this->_facebook_user; //Zend_Debug::dump($user); if($user) {
		 * $this->view->facebook_user = $user; //$access_token =
		 * $this->facebook_user->facebook_user_access_token; //$this->view->feed
		 * = $this->getFeed($access_token); }else { $this->view->facebook_user =
		 * null; }
		 */
		// $this->view->fanpage_id = $this->_fanpageId;
		$model = new Model_Rankings ();
		...
		if (! empty ( $this->_fanpageId )) {
			...
			try {
				// Check to see if the $fanpageId is cached and look it up if
				// not
				if (isset ( $cache ) && ! $cache->load ( $this->_fanpageId )) {
					...
					// $topPosts =
					// $model->getTopPosts($this->data['page']['id'], 5);
					...
					// Save to the cache, so we don't have to look it up next
					// time
		...
		if (! empty ( $this->_fanpageId ) && ! empty ( $this->_userId )) {
			...
			try {
				...
				if (isset ( $cache ) && ! $cache->load ( $this->_fanpageId . '_' . $this->_userId )) {
					...
					// $latestPost = $post
					// ->getLatestPost($this->data['page']['id'],5);
	...
	public function toplistAction() {
				...
				switch ($list) {
					...
					default :
						if (isset ( $cache ) && ! $cache->load ( $this->_fanpageId . '_topfan' )) {
							...
							// $toplist =
							// $model->getTopFansByWeek($this->_fanpageId, 5);
		...
		if ($list == 'top-fan') {
			...
			foreach ( $toplist as $top ) {
				// echo $top['facebook_user_id'];
				// $topArray[$count] = $follow->getRelation($this->_userId,
				// $top['facebook_user_id'],$this->_fanpageId);
				// echo $topArray[$count];
		...
		/*
		 * if(isset($this->_fanpageProfile->fanpage_level) &&
		 * $this->_fanpageProfile->fanpage_level < 3) { for ($i=0;
		 * $i<count($toplist); $i++){ $toplist[$i]['count'] = '?'; } if
		 * 	($userBoardData !=null) { $userBoardData['count'] = '?'; } }
		 */
	...
	public function leaderboardAction() {
		...
		// $fanpage['mostPopular'] =
		// $model->getMostPopularByWeek($this->_fanpageId, 5);
		// //Zend_Debug::dump($mostPopular);
		
		// $fanpage['topTalker'] = $model->getTopTalkerByWeek($this->_fanpageId,
		// 5);
		// //Zend_Debug::dump($topTalker);
		
		// $fanpage['topClicker'] =
		// $model->getTopClickerByWeek($this->_fanpageId, 5);
		// //Zend_Debug::dump($topClicker);
		
		// //$topPosts = $model->getTopPosts($this->data['page']['id'], 5);
		// $fanpage['topFollowed'] =
		// $model->getTopFollowedByWeek($this->_fanpageId, 5);
		// //$latestPost = $post ->getLatestPost($this->data['page']['id'],5);
		// $fanpage['topFansAllTime'] = $model->getTopFans($this->_fanpageId,
		// 5);
		// //Save to the cache, so we don't have to look it up next time
		// $cache->save($fanpage, $this->_fanpageId);
		// }else {
		...
		// }
		// } catch (Exception $e) {
		// Zend_Registry::get('appLogger')->log($e->getMessage() .' '
		// .$e->getCode(), Zend_Log::NOTICE, 'memcache info');
		// //echo $e->getMessage();
		// }
		// }
		...
		// $stat = $stat_model ->findFanRecord($this->_fanpageId,
		// $top['facebook_user_id']);
		
		// $topFanStats[$count]['total_posts'] = $stat[0]['total_posts'];
		// $topFanStats[$count]['total_comments'] = $stat[0]['total_comments'];
		// $topFanStats[$count]['total_likes'] = $stat[0]['total_likes'];
		// $topFanStats[$count]['total_get_comments'] =
		// $stat[0]['total_get_comments'];
		// $topFanStats[$count]['total_get_likes'] =
		// $stat[0]['total_get_likes'];
		...
		// //echo $top['facebook_user_id'];
		// $popularArray[$count] = $follow->getRelation($this->_userId,
		// $mp['facebook_user_id'],$this->_fanpageId);
		// //echo $topArray[$count];
		// $count++;
		...
		// //echo $top['facebook_user_id'];
		// $talkerArray[$count] = $follow->getRelation($this->_userId,
		// $tt['facebook_user_id'],$this->_fanpageId);
		// //echo $topArray[$count];
		// $count++;
		...
		// //echo $top['facebook_user_id'];
		// $clickerArray[$count] = $follow->getRelation($this->_userId,
		// $tc['facebook_user_id'],$this->_fanpageId);
		// //echo $topArray[$count];
		// $count++;
		...
		// //echo $top['facebook_user_id'];
		// $followedArray[$count] = $follow->getRelation($this->_userId,
		// $tf['facebook_user_id'],$this->_fanpageId);
		// //echo $topArray[$count];
		// $count++;
		...
		// //echo $top['facebook_user_id'];
		// $topFansAllTimeArray[$count] = $follow->getRelation($this->_userId,
		// $top['facebook_user_id'],$this->_fanpageId);
		// //echo $topArray[$count];
		...
		// $topFansAllTimeStats[$count]['total_posts'] =
		// $stat[0]['total_posts'];
		// $topFansAllTimeStats[$count]['total_comments'] =
		// $stat[0]['total_comments'];
		// $topFansAllTimeStats[$count]['total_likes'] =
		// $stat[0]['total_likes'];
		// $topFansAllTimeStats[$count]['total_get_comments'] =
		// $stat[0]['total_get_comments'];
		// $topFansAllTimeStats[$count]['total_get_likes'] =
		// $stat[0]['total_get_likes'];
		...
		// //Check to see if the $fanpageId is cached and look it up if not
		// if(isset($cache) && !$cache->load($this->_fanpageId .'_'
		// .$this->_userId)){
		// //Look up the $fanpageId
		// $userLeaderBoardData['topFans'] =
		// $model->getUserTopFansRank($this->_fanpageId, $this->_userId);
		
		// $userLeaderBoardData['mostPopular'] =
		// $model->getUserMostPopularRank($this->_fanpageId, $this->_userId);
		
		// $userLeaderBoardData['topTalker'] =
		// $model->getUserTopTalkerRank($this->_fanpageId, $this->_userId);
		// //Zend_Debug::dump($topTalker);
		
		// $userLeaderBoardData['topClicker'] =
		// $model->getUserTopClickerRank($this->_fanpageId,$this->_userId);
		// //Zend_Debug::dump($topClicker);
		
		// //$topPosts = $model->getTopPosts($this->data['page']['id'], 5);
		// $userLeaderBoardData['topFollowed'] =
		// $model->getTopFollowedRankByWeek($this->_fanpageId, $this->_userId);
		// //$latestPost = $post ->getLatestPost($this->data['page']['id'],5);
		// $userLeaderBoardData['topFansAllTime'] =
		// $model->getUserTopFansRank($this->_fanpageId, $this->_userId);
		// //Save to the cache, so we don't have to look it up next time
		// $cache->save($userLeaderBoardData, $this->_fanpageId .'_'
		// .$this->_userId);
		// }else {
		// //echo 'memcache look up';
		// $userLeaderBoardData = $cache->load($this->_fanpageId .'_'
		// .$this->_userId);
		// }
		// } catch (Exception $e) {
		// Zend_Registry::get('appLogger')->log($e->getMessage() .' '
		...
		// if(isset($this->_fanpageProfile->fanpage_level) &&
		// $this->_fanpageProfile->fanpage_level < 3) {
	...
	public function newsfeedAction() {
		...
		// $post = $post->getMyFeedPost($this->_fanpageId, $this->_userId , 10,
		// null);
		// $feed = array();
		// $feed = array_merge($this->getPostLikesByBatch(array_slice($post,
		// 0,5)),$this->getPostLikesByBatch(array_slice($post, 5,5)));
		...
		if ($latest != null) {
			foreach ( $latest as $l ) {
				if (isset ( $l->story )) {
					/*
					 * $cache = Zend_Registry::get('memcache');
					 * $cache->setLifetime(1800); try { $adminPostId = $l->id;
					 * $cache->save($l, $adminPostId); //$x =
					 * $cache->load($adminPostId); //Zend_Debug::dump($x); }
					 * catch (Exception $e) {
					 * Zend_Registry::get('appLogger')->log($e->getMessage() .'
					 * ' .$e->getCode(), Zend_Log::NOTICE, 'memcache info');
					 * //echo $e->getMessage(); }
					...
					if (isset ( $l->likes )) {
						foreach ( $l->likes->data as $x ) {
							...
							if ($x->id == $this->_userId) {
								$latestlike = 1;
								// echo "$latestlike[$count] in the condensed
							// list";
							}
		...
		// }
		// if($likes[$count]==0){
		// $likes[$count] = $likesModel->getLikes($this->_fanpageId, $posts->id,
		// $this->_userId );
		// //Zend_Debug::dump($likes[$count]);
		// }
		// }
		
		// $relation[$count] = $follow->getRelation($this->_userId,
		// $posts->from->id,$this->_fanpageId);
		// //echo $likes[$count];
		// $yourpoints[$count] = 0;
		// //$points = new Model_PointLog();
		
		// //$points = $points->getPointsByPost($this->_fanpageId,
		// $this->_userId, $posts->id );
		// //echo $posts->id;
		...
		// $comment_likes[$count2] = $likesModel->getLikes($this->_fanpageId,
		// $c->id, $this->_userId );
		// $comment_relation[$count2] = $follow->getRelation($this->_userId,
		// $c->from->id,$this->_fanpageId);
	 ...
	 * $this->_helper->layout->disableLayout();
	 * //$this->_helper->viewRenderer->setNoRender(true); $post = new
	 * Model_Posts; $latestPost = $post ->getLatestPost($this->_fanpageId,5);
	 * $likesModel = new Model_Likes(); $likes = array(); $count=0; foreach
	 * ($latestPost as $posts){ //echo $top['facebook_user_id']; $likes[$count]
	 * = $likesModel->getLikes($this->_fanpageId, $posts['post_id'],
	 * $this->_userId ); //echo $likes[$count]; $count++; } $this->view->likes =
	 * $likes; $this->view->latest_post = $latestPost;
	...
	public function awardsAction() {
		...
		// $userBadgeCount = $userBadges->getNumBadgesByUser($this->_fanpageId,
		...
		// $overallAchievement =
		// $userBadgeCount[0]['count']/$allBadges[0]['count']*100;
	...
	public function redeemAction() {
		...
		// $badgesList = array();
		// if(!empty($userLeaderBoardData['topFans']['my_rank']) &&
		// $userLeaderBoardData['topFans']['my_rank'] < 5) {
		// $badgeModel = new Model_Badges();
		// $badgesList =
		// $badgeModel->findByFanpageLevel($this->_fanpageProfile->fanpage_level,
		// $this->_fanpageId, $user->facebook_user_id);
		// }
	...
	public function myprofileAction() {
		...
		/*
		 * if($user) { $this->view->facebook_user = $user; //$access_token =
		 * $this->facebook_user->facebook_user_access_token; //$this->view->feed
		 * = $this->getFeed($access_token); // aplly memcache $cache =
		 * Zend_Registry::get('memcache'); $cache->setLifetime(1800); $fan =
		 * null; try { $fanProfileId = $this->_fanpageId .'_'
		 * .$user->facebook_user_id .'_fan'; //$cache->remove($fanProfileId);
		 * //Check to see if the $fanpageId is cached and look it up if not
		 * if(isset($cache) && !$cache->load($fanProfileId)){ //echo 'db look
		 * up'; $fan = new Model_Fans($user->facebook_user_id,
		 * $this->_fanpageId); $fan = $fan->getFanProfile(); //Save to the
		 * cache, so we don't have to look it up next time $cache->save($fan,
		 * $fanProfileId); }else { //echo 'memcache look up'; $fan =
		 * $cache->load($fanProfileId); } } catch (Exception $e) {
		 * Zend_Registry::get('appLogger')->log($e->getMessage() .' '
		 * .$e->getCode(), Zend_Log::NOTICE, 'memcache info'); //echo
		 * $e->getMessage(); } }else { $this->view->facebook_user = null; }
		 */
		...
		// $badges2 = $badgesModel ->
		// getBadgesByFanpageIdAndFanIDUnread($this->_fanpageId, $this->_userId,
		// false);
	...
	public function userprofileAction() {
		...
		$user = $user->find ( $this->_request->getParam ( 'target' ) )->current (); // the
		                                                                      // target
		                                                                      // Zend_Debug::dump($user);
		if ($user) {
			$this->view->facebook_user = $user;
		...
		$activity = $activitiesModel->getRecentActivities ( $user->facebook_user_id, $this->_fanpageId, 20 ); // $activity->getUserActivity($this->_fanpageId,
		                                                                                                   // $user->facebook_user_id,
		                                                                                                   // 15);
	...
	public function fancrankfeedAction() {
		...
		if ($result != null) {
			...
			/*
			 * foreach ($result as $posts){ //Zend_Debug::dump($posts); //echo
			 * $top['facebook_user_id']; $likes[$count] =
			 * $likesModel->getLikes($this->_fanpageId, $posts['post_id'],
			 * $this->_userId ); $relation[$count] =
			 * $follow->getRelation($this->_userId,
			 * $posts['facebook_user_id'],$this->_fanpageId); //echo
			 * $likes[$count]; $count++; } }else{
			 */
			foreach ( $result as $posts ) {
				// echo $top['facebook_user_id'];
				if ($posts != null) {
		...
		/*
		 * if($viewAs == 'myfeed'){ $this->view->myfeedcount = $count;
		 * $this->render("fancrankfeedm"); }else{
		 */
	...
	protected function postPointsCalculate($posts) {
		...
		/*
		 * //Zend_Debug::dump( $points); //if this is your post
		 * if($this->_userId == $posts->from->id){ $yourpoints -= 5;
		 * $uniquereply = array(); //if your posts has comments and don't need
		 * additional api call if (isset($posts->comments->data)){ if
		 * (count($posts->comments->data) == $posts->comments->count){
		 * foreach($posts->comments->data as $c){ //echo
		 * $posts->id.'-'.$c->from->id; // if comment not from you, add points
		 * if ($c->from->id != $this->_userId){ $yourpoints +=2; $uniquereply[]
		 * = $c->from->id; }else{ //if comment is users, check for likes if
		 * (isset($c->like_count)){ if ($c->like_count != 0){ $ls =
		 * $this->getPostLikes($posts->id, $c->like_count); foreach($ls as $l){
		 * if($l->id != $this->_userId){ $yourpoints +=1; } } } } } }
		 * }else{//api called required to get other comments $comm =
		 * $this->getFeedComment($posts->id, $posts->comments->count);
		 * foreach($comm as $c){ //echo $posts->id.'-'.$c->from->id; if
		 * ($c->from->id != $this->_userId){ $yourpoints +=2; $uniquereply[] =
		 * $c->from->id; }else{ if (isset($c->like_count)){ if ($c->like_count
		 * != 0){ $ls = $this->getPostLikes($posts->id, $c->like_count);
		 * foreach($ls as $l){ if($l->id != $this->_userId){ $yourpoints +=1; }
		 * } } } } } } } //if your post has likes if (isset($posts->likes->data
		 * )){ // if dont need api call for likes list if
		 * (count($posts->likes->data) == $posts->likes->count){ foreach
		 * ($posts->likes->data as $l){ if($l->id != $this->_userId){
		 * $yourpoints +=1; $uniquereply[] = $l->id; } } }else{ // if need api
		 * call for complete likes list $ls = $this->getPostLikes($posts->id,
		 * $posts->likes->count); foreach($ls as $l){ if($l->id !=
		 * $this->_userId){ $yourpoints +=1; } } } } //apply unique count and
		 * virginity $uniquereply = array_unique($uniquereply);
		 * //Zend_Debug::dump($uniquereply); $yourpoints += count($uniquereply);
		 * if (count($uniquereply)>0){ $yourpoints += 4; } //if post is by admin
		 * }else if ($this->_fanpageId == $posts->from->id)	{ // does the admin
		 * post have comments if (isset($posts->comments->data)){ $limit = 5; //
		 * limits of 5 for getting time bonus $post_time = new
		 * Zend_Date($posts->created_time); //do we need extra api call for
		 * admin comments if (count($posts->comments->data) ==
		 * $posts->comments->count){ foreach($posts->comments->data as $c){
		 * //echo $posts->id.'-'.$c->from->id; if ($c->from->id ==
		 * $this->_userId){ $yourpoints +=2; $comment_time = new
		 * Zend_date($c->created_time); //Zend_Debug::dump($post_time); //give
		 * bonus points for before 1 hour difference $comment_time->sub('1',
		 * Zend_Date::HOUR); if ($comment_time->isEarlier($post_time)){
		 * $yourpoints += ($limit>0)?$limit:0; $limit--; } //did your comment
		 * get likes if (isset($c->like_count)){ if ($c->like_count != 0){ $ls =
		 * $this->getPostLikes($posts->id, $c->like_count); foreach($ls as $l){
		 * if($l->id != $this->_userId){ $yourpoints +=1; } } } } } } }else{
		 * $comm = $this->getFeedComment($posts->id, $posts->comments->count);
		 * foreach($comm as $c){ //echo $posts->id.'-'.$c->from->id; if
		 * ($c->from->id == $this->_userId){ $yourpoints +=2; $comment_time =
		 * new Zend_date($c->created_time); //Zend_Debug::dump($post_time);
		 * $comment_time->sub('1', Zend_Date::HOUR); if
		 * ($comment_time->isEarlier($post_time)){ $yourpoints +=
		 * ($limit>0)?$limit:0; $limit--; } //did comment get likes if
		 * (isset($c->like_count)){ if ($c->like_count != 0){ $ls =
		 * $this->getPostLikes($posts->id, $c->like_count); foreach($ls as $l){
		 * if($l->id != $this->_userId){ $yourpoints +=1; } } } } } } } }
		 * //////LIKE TIME BONUS IS STILL MISSING YO. if
		 * (isset($posts->likes->data )){ if (count($posts->likes->data) ==
		 * $posts->likes->count){ foreach ($posts->likes->data as $l){ if($l->id
		 * == $this->_userId){ $yourpoints +=1; } } }else{ $ls =
		 * $this->getPostLikes($posts->id, $posts->likes->count); foreach($ls as
		 * $l){ if($l->id == $this->_userId){ $yourpoints +=1; } } } }
		 * }else{//for other user posts //did you like it? if
		 * (isset($posts->likes->data )){ if (count($posts->likes->data) ==
		 * $posts->likes->count){ foreach ($posts->likes->data as $l){ if($l->id
		 * == $this->_userId){ $yourpoints +=1; } } }else{ $ls =
		 * $this->getPostLikes($posts->id,$posts->likes->count ); foreach($ls as
		 * $l){ if($l->id == $this->_userId){ $yourpoints +=1; } } } } // did
		 * you comment on a user post and get likes on your comments if
		 * (isset($posts->comments->data)){ if (count($posts->comments->data) ==
		 * $posts->comments->count){ foreach($posts->comments->data as $c){
		 * //echo $posts->id.'-'.$c->from->id; if ($c->from->id ==
		 * $this->_userId){ if (isset($c->like_count)){ if ($c->like_count !=
		 * 0){ $ls = $this->getPostLikes($posts->id, $c->like_count);
		 * foreach($ls as $l){ if($l->id != $this->_userId){ $yourpoints +=1; }
		 * } } } } } }else{ $comm = $this->getFeedComment($posts->id,
		 * $posts->comments->count); foreach($comm as $c){ //echo
		 * $posts->id.'-'.$c->from->id; if ($c->from->id == $this->_userId){ if
		 * (isset($c->like_count)){ if ($c->like_count != 0){ $ls =
		 * $this->getPostLikes($posts->id, $c->like_count); foreach($ls as $l){
		 * if($l->id != $this->_userId){ $yourpoints +=1; } } } } } } } } }
		 * //echo $posts->id.' - '.$yourpoints .'<br/>'; return $yourpoints;
		 */
	}
	...
	public function allactivitiesAction() {
		...
		try {
			...
			if (isset ( $cache ) && ! $cache->load ( $fanActivityId )) {
				// echo 'db look up';
				// $fan = new Model_Fans($user->facebook_user_id,
				// $this->_fanpageId);
				...
				if (! empty ( $activities [0] ['created_time'] )) {
					// $newActivity =
				// $activitiesModel->getAllActivitiesSince($this->_fanpageId,
				// $limit, $activities[0]['created_time']);
				}
	...
	public function upcomingbadgesAction() {
		...
		// $badges = $this->badgeArray($this->_fanpageId, $this->_userId,
		// $limit);
		// Zend_Debug::dump($badges);
		// exit();
	...
	// this grabs the comments for a specific post and returns the data to
	// fancrankfeedcommentAction
	...
	protected function getPostLikesByBatch($posts) {
		...
		/*
		 * if ( !empty($allpost)){ foreach ($allpost as $likes){ $updatingPost =
		 * new Model_Posts(); if (isset($likes->id)){ $updatingPost =
		 * $updatingPost -> findPost($likes->id); if(!empty ($updatingPost)){ if
		 * (isset($likes->likes->count) && ($updatingPost['post_likes_count'] !=
		 * $likes->likes->count)){ $updatingPost ->post_likes_count =
		 * $likes->likes->count; } if (isset($likes->comments->count) &&
		 * ($updatingPost['post_comments_count'] != $likes->comments->count)){
		 * $updatingPost ->post_comments_count = $likes->comments->count; }
		 * $updatingPost->save(); } } if (isset($likes->likes)){ $likeslist[] =
		 * $likes->likes->data; }else{ $likeslist[] = array(); } } } return
		 * null;
		 */
	}
	...
	// this grabs the facebook feed for the current fanpage id and returns the
	// list to fancrankAction
	protected function getFeed($until, $view) {
		...
		switch ($view) {
			...
			case 'all' :
				...
				return $result->data;
			/*
			 * case 'user': return $this->feedFilterByUser($result->data,
			 * $this->_fanpageId);
			 */
			case 'myfeed' :
				$post = new Model_Posts ();
	...
	/*
	 * protected function feedFilterByAdmin($data, $fanpageId) { $result =
	 * array(); foreach ($data as $value) { if(!empty($value->from->id) &&
	 * $value->from->id === $fanpageId) { $result[] = $value; } } return
	 * $result; }
	 */
	 ...
	 * public function fancrankfeedAction() {
	 * $this->_helper->layout->disableLayout();
	 * //$this->_helper->viewRenderer->setNoRender(true); $viewAs =
	 * $this->_request->getParam('viewAs'); $result = array(); $result =
	 * $this->getFeed($this->_request->getParam('fanpage_id'),
	 * $this->_request->getParam('access_token'),8, $viewAs); //$result =
	 * json_encode($result); //Zend_Debug::dump($result); $this->view->post =
	 * $result; $this->render("fancrankfeed"); }
	 */
	 ...
	 * $this->_helper->layout->disableLayout();
	 * $this->_helper->viewRenderer->setNoRender(true); $limit =
	 * $this->_request->getParam('limit'); $target =
	 * $this->_request->getParam('target'); $userName = $this->
	 * _request->getParam('targetname'); $follow = new Model_Subscribes();
	 * $result= $follow->getFriendsList($target, $this->_fanpageId, $limit);
	 * $relation = array(); $count = 0; foreach ($result as $r){ //echo
	 * $top['facebook_user_id']; $relation[$count] =
	 * $follow->getRelation($this->_userId, $r['facebook_user_id']); //echo
	 * $topArray[$count]; $count++; } //Zend_Debug::dump($result);
	 * $this->view->relation= $relation; $this->view->result = $result;
	 * $this->view->title = 'Friends'; $this->view->user_name = $userName;
	 * $this->view->user_id = $target; $this->render("userlist"); }
	 */
	 ...
	 * $this->_helper->layout->disableLayout();
	 * $this->_helper->viewRenderer->setNoRender(true); $result =
	 * $this->getAdminFeed($this->_getParam('id'),
	 * $this->_getParam('access_token'), 5); $this->_helper->json($result); }
	 */
	public function logoutAction() {
		...
		// $this->getRequest()->getModuleName(), array($this->_getParam('id') =>
		// null));
		// $this->_helper->redirector('index', 'index', 'app',
		// array($this->_getParam('id') => ''));
	...
	protected function badgeArray2D($fanpage_id, $facebook_user_id, $limit) {
		...
		/*
		 * $cg10s = $fan->getNumberOfCommentOnPostInTimeByUser($fanpage_id,
		 * $facebook_user_id, 'interval 10 second', 'all'); $cg1m =
		 * $fan->getNumberOfCommentOnPostInTimeByUser($fanpage_id,
		 * $facebook_user_id, 'interval 1 minute', 'all'); $cl10s =
		 * $fan->getNumberOfCommentOnPostInTimeByUser($fanpage_id,
		 * $facebook_user_id, 'interval 10 second', 'link'); $cl1m =
		 * $fan->getNumberOfCommentOnPostInTimeByUser($fanpage_id,
		 * $facebook_user_id, 'interval 1 minute', 'link'); $cp10s=
		 * $fan->getNumberOfCommentOnPostInTimeByUser($fanpage_id,
		 * $facebook_user_id, 'interval 10 second', 'photo'); $cp1m =
		 * $fan->getNumberOfCommentOnPostInTimeByUser($fanpage_id,
		 * $facebook_user_id, 'interval 1 minute', 'photo'); $cs10s=
		 * $fan->getNumberOfCommentOnPostInTimeByUser($fanpage_id,
		 * $facebook_user_id, 'interval 10 second', 'status'); $cs1m =
		 * $fan->getNumberOfCommentOnPostInTimeByUser($fanpage_id,
		 * $facebook_user_id, 'interval 1 minute', 'status'); $cv10s =
		 * $fan->getNumberOfCommentOnPostInTimeByUser($fanpage_id,
		 * $facebook_user_id, 'interval 10 second', 'video'); $cv1m =
		 * $fan->getNumberOfCommentOnPostInTimeByUser($fanpage_id,
		 * $facebook_user_id, 'interval 1 minute', 'video'); $gcg10s =
		 * $fan->getNumberOfCommentOnPostInTimeRecieved($fanpage_id,
		 * $facebook_user_id, 'interval 10 second', 'all'); $gcg1m =
		 * $fan->getNumberOfCommentOnPostInTimeRecieved($fanpage_id,
		 * $facebook_user_id, 'interval 1 minute', 'all'); $gcl10s =
		 * $fan->getNumberOfCommentOnPostInTimeRecieved($fanpage_id,
		 * $facebook_user_id, 'interval 10 second', 'link'); $gcg1m =
		 * $fan->getNumberOfCommentOnPostInTimeRecieved($fanpage_id,
		 * $facebook_user_id, 'interval 1 minute', 'link'); $gcp10s =
		 * $fan->getNumberOfCommentOnPostInTimeRecieved($fanpage_id,
		 * $facebook_user_id, 'interval 10 second', 'photo'); $gcp1m =
		 * $fan->getNumberOfCommentOnPostInTimeRecieved($fanpage_id,
		 * $facebook_user_id, 'interval 1 minute', 'photo'); $gcs10s =
		 * $fan->getNumberOfCommentOnPostInTimeRecieved($fanpage_id,
		 * $facebook_user_id, 'interval 10 second', 'status'); $gcs1m =
		 * $fan->getNumberOfCommentOnPostInTimeRecieved($fanpage_id,
		 * $facebook_user_id, 'interval 1 minute', 'status'); $gcv10s =
		 * $fan->getNumberOfCommentOnPostInTimeRecieved($fanpage_id,
		 * $facebook_user_id, 'interval 10 second', 'status'); $gcv1m=
		 * $fan->getNumberOfCommentOnPostInTimeRecieved($fanpage_id,
		 * $facebook_user_id, 'interval 1 minute', 'status'); $lg10s =
		 * $fan->getNumberOfLikesOnPostInTimeByUser($fanpage_id,
		 * $facebook_user_id, 'interval 10 second', 'all'); $lg1m =
		 * $fan->getNumberOfLikesOnPostInTimeByUser($fanpage_id,
		 * $facebook_user_id, 'interval 1 minute', 'all'); $lp10s =
		 * $fan->getNumberOfLikesOnPostInTimeByUser($fanpage_id,
		 * $facebook_user_id, 'interval 10 second', 'photo'); $ll1m =
		 * $fan->getNumberOfLikesOnPostInTimeByUser($fanpage_id,
		 * $facebook_user_id, 'interval 1 minute', 'link'); $ll10s =
		 * $fan->getNumberOfLikesOnPostInTimeByUser($fanpage_id,
		 * $facebook_user_id, 'interval 10 second', 'link'); $lp1m =
		 * $fan->getNumberOfLikesOnPostInTimeByUser($fanpage_id,
		 * $facebook_user_id, 'interval 1 minute', 'photo'); $ls10s =
		 * $fan->getNumberOfLikesOnPostInTimeByUser($fanpage_id,
		 * $facebook_user_id, 'interval 10 second', 'status'); $ls1m =
		 * $fan->getNumberOfLikesOnPostInTimeByUser($fanpage_id,
		 * $facebook_user_id, 'interval 1 minute', 'status'); $lv10s =
		 * $fan->getNumberOfLikesOnPostInTimeByUser($fanpage_id,
		 * $facebook_user_id, 'interval 10 second', 'video'); $lv1m =
		 * $fan->getNumberOfLikesOnPostInTimeByUser($fanpage_id,
		 * $facebook_user_id, 'interval 1 minute', 'video'); $lc10s =
		 * $fan->getNumberOfLikesOnPostInTimeByUser($fanpage_id,
		 * $facebook_user_id, 'interval 10 second', 'comment'); $lc1m =
		 * $fan->getNumberOfLikesOnPostInTimeByUser($fanpage_id,
		 * $facebook_user_id, 'interval 1 minute', 'comment'); $glg10s =
		 * $fan->getNumberOfLikesOnPostInTimeRecieved($fanpage_id,
		 * $facebook_user_id, 'interval 10 second', 'all'); $glg1m =
		 * $fan->getNumberOfLikesOnPostInTimeRecieved($fanpage_id,
		 * $facebook_user_id, 'interval 1 minute', 'all'); $glg10s =
		 * $fan->getNumberOfLikesOnPostInTimeRecieved($fanpage_id,
		 * $facebook_user_id, 'interval 10 second', 'link'); $gll1m =
		 * $fan->getNumberOfLikesOnPostInTimeRecieved($fanpage_id,
		 * $facebook_user_id, 'interval 1 minute', 'link'); $glp10s =
		 * $fan->getNumberOfLikesOnPostInTimeRecieved($fanpage_id,
		 * $facebook_user_id, 'interval 10 second', 'photo'); $glp1m =
		 * $fan->getNumberOfLikesOnPostInTimeRecieved($fanpage_id,
		 * $facebook_user_id, 'interval 1 minute', 'photo'); $gls10s =
		 * $fan->getNumberOfLikesOnPostInTimeRecieved($fanpage_id,
		 * $facebook_user_id, 'interval 10 second', 'status'); $gls1m =
		 * $fan->getNumberOfLikesOnPostInTimeRecieved($fanpage_id,
		 * $facebook_user_id, 'interval 1 minute', 'status'); $glv10s =
		 * $fan->getNumberOfLikesOnPostInTimeRecieved($fanpage_id,
		 * $facebook_user_id, 'interval 10 second', 'video'); $glv1m =
		 * $fan->getNumberOfLikesOnPostInTimeRecieved($fanpage_id,
		 * $facebook_user_id, 'interval 1 minute', 'video');
		 */
		...
		foreach ( $badge as $b ) {
			...
			switch ($b ['name']) {
				case 'Comment-General' :
					// $temp = $fan->getTotalComments($fanpage_id,
					// $facebook_user_id);
				...
				case 'Comment-General-1min' :
					...
					break;
				case 'Comment-Link' :
					// $temp = $fan->getLinkComments($fanpage_id,
					// $facebook_user_id);
				...
				case 'Comment-Link-1min' :
					...
					break;
				case 'Comment-Photo' :
					// $temp = $fan->getPhotoComments($fanpage_id,
					// $facebook_user_id);
				...
				case 'Comment-Photo-1min' :
					...
					break;
				case 'Comment-Status' :
					// $temp = $fan->getStatusComments($fanpage_id,
					// $facebook_user_id);
				...
				case 'Comment-Status-1min' :
					...
					break;
				case 'Comment-Video' :
					// $temp = $fan->getVideoComments($fanpage_id,
					// $facebook_user_id);
				...
				case 'Get-Comment-General' :
					// $temp = $fan->getTotalGetComments($fanpage_id,
					// $facebook_user_id);
				...
				case 'Get-Comment-Link' :
					// $temp = $fan->getLinkGetComments($fanpage_id,
					// $facebook_user_id);
				...
				case 'Get-Comment-Photo' :
					// $temp = $fan->getPhotoGetComments($fanpage_id,
					// $facebook_user_id);
				...
				case 'Get-Comment-Status' :
					// $temp = $fan->getStatusGetComments($fanpage_id,
					// $facebook_user_id);
				...
				case 'Get-Comment-Video' :
					// $temp = $fan->getVideoGetComments($fanpage_id,
					// $facebook_user_id);
				...
				case 'Get-Comment-Video-1min' :
					...
					break;
				case 'Like-General' :
					// $temp = $fan->getTotalLikes($fanpage_id,
					// $facebook_user_id);
				...
				case 'Like-General-1min' :
					...
					break;
				case 'Like-Link' :
					// $temp = $fan->getLinkLikes($fanpage_id,
					// $facebook_user_id);
				...
				case 'Like-Link-1min' :
					...
					break;
				case 'Like-Photo' :
					// $temp = $fan->getPhotoLikes($fanpage_id,
					// $facebook_user_id);
				...
				case 'Like-Photo-1min' :
					...
					break;
				case 'Like-Status' :
					// $temp = $fan->getStatusLikes($fanpage_id,
					// $facebook_user_id);
				...
				case 'Like-Status-1min' :
					...
					break;
				case 'Like-Video' :
					// $temp = $fan->getVideoLikes($fanpage_id,
					// $facebook_user_id);
				...
				case 'Multiple-Comment' :
					...
					$temp = (($timearray ['comments'] ['video'] ['no-time'] / $b ['quantity'] > 1) ? 1 : $timearray ['comments'] ['video'] ['no-time'] / $b ['quantity']) + (($timearray ['comments'] ['status'] ['no-time'] / $b ['quantity'] > 1) ? 1 : $timearray ['comments'] ['status'] ['no-time'] / $b ['quantity']) + (($timearray ['comments'] ['photo'] ['no-time'] / $b ['quantity'] > 1) ? 1 : $timearray ['comments'] ['photo'] ['no-time'] / $b ['quantity']) + (($timearray ['comments'] ['link'] ['no-time'] / $b ['quantity'] > 1) ? 1 : $timearray ['comments'] ['link'] ['no-time'] / $b ['quantity']) + (($timearray ['comments'] ['all'] ['no-time'] / $b ['quantity'] > 1) ? 1 : $timearray ['comments'] ['all'] ['no-time'] / $b ['quantity']);
				...
				case 'Multiple-Like' :
					...
					$temp = (($timearray ['likes'] ['video'] ['no-time'] / $b ['quantity'] > 1) ? 1 : $timearray ['likes'] ['video'] ['no-time'] / $b ['quantity']) + (($timearray ['likes'] ['status'] ['no-time'] / $b ['quantity'] > 1) ? 1 : $timearray ['likes'] ['status'] ['no-time'] / $b ['quantity']) + (($timearray ['likes'] ['photo'] ['no-time'] / $b ['quantity'] > 1) ? 1 : $timearray ['likes'] ['photo'] ['no-time'] / $b ['quantity']) + (($timearray ['likes'] ['link'] ['no-time'] / $b ['quantity'] > 1) ? 1 : $timearray ['likes'] ['link'] ['no-time'] / $b ['quantity']) + (($timearray ['likes'] ['all'] ['no-time'] / $b ['quantity'] > 1) ? 1 : $timearray ['likes'] ['all'] ['no-time'] / $b ['quantity']);
					// Zend_Debug::dump($temp);
					$temp = round ( $temp / 5, 2, PHP_ROUND_HALF_DOWN );
					$array [$count] ['percentage'] = $temp;
					break;
				...
				/*
				 * case 'Fan-Favorite-Month': $array[$count]['percentage'] =
				 * 'none';break; case 'Fan-Favorite-Week':
				 * $array[$count]['percentage'] ='none';break; case
				 * 'Fan-Favorite-Year':$array[$count]['percentage'] ='none';
				 * break; case 'Top-Clicker-Month': $array[$count]['percentage']
				 * ='none'; break; case
				 * 'Top-Clicker-Week':$array[$count]['percentage'] ='none';
				 * break; case 'Top-Clicker-Year': $array[$count]['percentage']
				 * ='none'; break; case
				 * 'Top-Fan-Month':$array[$count]['percentage'] ='none'; break;
				 * case 'Top-Fan-Week':$array[$count]['percentage'] ='none';
				 * break; case 'Top-Fan-Year':$array[$count]['percentage']
				 * ='none'; break; case
				 * 'Top-Talker-Month':$array[$count]['percentage'] ='none';
				 * break; case 'Top-Talker-Week': $array[$count]['percentage']
				 * ='none'; break; case
				 * 'Top-Talker-Year':$array[$count]['percentage'] ='none';
				 * break; case 'Watched-Tutorial':$array[$count]['percentage']
				 * ='none'; break;
				 */
				case 'default' :
					break;
	...
	protected function badgeArray($fanpage_id, $facebook_user_id) {
		...
		foreach ( $badge as $b ) {
			...
			switch ($b ['name']) {
				...
				case 'Multiple-Comment' :
					$temp = $array ['Comment-General'] [( string ) $b ['quantity']] ['percentage'] + $array ['Comment-Status'] [( string ) $b ['quantity']] ['percentage'] + $array ['Comment-Photo'] [( string ) $b ['quantity']] ['percentage'] + $array ['Comment-Video'] [( string ) $b ['quantity']] ['percentage'] + $array ['Comment-Link'] [( string ) $b ['quantity']] ['percentage'];
					// Zend_Debug::dump($temp);
					$temp = $temp / 5;
					$array [$b ['name']] [( string ) $b ['quantity']] ['percentage'] = $temp;
					break;
				...
				case 'Multiple-Like' :
					$temp = $array ['Like-General'] [( string ) $b ['quantity']] ['percentage'] + $array ['Like-Status'] [( string ) $b ['quantity']] ['percentage'] + $array ['Like-Photo'] [( string ) $b ['quantity']] ['percentage'] + $array ['Like-Video'] [( string ) $b ['quantity']] ['percentage'] + $array ['Like-Link'] [( string ) $b ['quantity']] ['percentage'];
					// Zend_Debug::dump($temp);
					$temp = $temp / 5;
					$array [$b ['name']] [( string ) $b ['quantity']] ['percentage'] = $temp;
					break;
				...
				/*
				 * case 'Fan-Favorite-Month':
				 * $array[$b['name']][(string)$b['quantity']]['percentage'] =
				 * 'none';break; case 'Fan-Favorite-Week':
				 * $array[$b['name']][(string)$b['quantity']]['percentage']
				 * ='none';break; case
				 * 'Fan-Favorite-Year':$array[$b['name']][(string)$b['quantity']]['percentage']
				 * ='none'; break; case 'Top-Clicker-Month':
				 * $array[$b['name']][(string)$b['quantity']]['percentage']
				 * ='none'; break; case
				 * 'Top-Clicker-Week':$array[$b['name']][(string)$b['quantity']]['percentage']
				 * ='none'; break; case 'Top-Clicker-Year':
				 * $array[$b['name']][(string)$b['quantity']]['percentage']
				 * ='none'; break; case
				 * 'Top-Fan-Month':$array[$b['name']][(string)$b['quantity']]['percentage']
				 * ='none'; break; case
				 * 'Top-Fan-Week':$array[$b['name']][(string)$b['quantity']]['percentage']
				 * ='none'; break; case
				 * 'Top-Fan-Year':$array[$b['name']][(string)$b['quantity']]['percentage']
				 * ='none'; break; case
				 * 'Top-Talker-Month':$array[$b['name']][(string)$b['quantity']]['percentage']
				 * ='none'; break; case 'Top-Talker-Week':
				 * $array[$b['name']][(string)$b['quantity']]['percentage']
				 * ='none'; break; case
				 * 'Top-Talker-Year':$array[$b['name']][(string)$b['quantity']]['percentage']
				 * ='none'; break; case
				 * 'Watched-Tutorial':$array[$b['name']][(string)$b['quantity']]['percentage']
				 * ='none'; break;
				 */
				case 'default' :
					break;
	...
	private function feedFirstQuery() {
		// $tmp[] = array('method'=>'GET', 'relative_url'=>
		// "/$this->_fanpageId/feed?limit=10");
		$tmp [] = array (
				'method' => 'GET',
				'relative_url' => "/$this->_fanpageId/posts?limit=10" 
		);
		$batchQueries = 'batch=' . urlencode ( json_encode ( $tmp ) ) . '&access_token=' . $this->_accessToken;
